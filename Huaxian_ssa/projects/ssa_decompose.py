...
data_path = root_path + '\\Huaxian_ssa\\data\\'
...
huaxian = huaxian['MonthlyRunoff']
# plotting the monthly runoff of huaxian station
huaxian.plot()
plt.title("Monthly Runoff of Huaxian station")
plt.xlabel("Time(1953/01-2008/12)")
plt.ylabel(r"Runoff($m^3/s$)")
plt.tight_layout()

# loading the training and entire monthly runoff
start = 24
stop = 696#576
full = huaxian[start:] #(full)from 1953/01 to 2018/12 792 samples
full = full.reset_index(drop=True)
train = huaxian[start:stop] #(train)from 1953/01 to 1998/12, 552 samples
train = train.reset_index(drop=True)

#%%
# Decompose the monthly runoff of huaxian
window = 12 #45% of 552
